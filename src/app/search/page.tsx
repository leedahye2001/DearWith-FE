...
import { useState } from "react";
import { getArtist } from "@/apis/api"; // 네가 준 getArtist 함수
interface Artist {
  id: number;
  nameKr: string;
  nameEn: string;
...
const Page = () => {
  ...
  const handleSearchChange = async (value: string) => {
    ...
    try {
      const res = await getArtist(value);
      setArtists(res || []);
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="bg-bg-1 dark:bg-bg-1 flex flex-col justify-center mt-[54px] gap-[12px] justify-center">
      <Input
        _value={search}
        _state="textbox-basic"
        _rightNode={<Search />}
      ...
      <div className="flex flex-col gap-[20px] mt-[12px] px-[24px] w-full">
        ...
        <div className="flex flex-col gap-[12px] max-h-[400px] overflow-y-auto w-full">
          {artists.length === 0 ? (
            ...
            artists.slice(0, 10).map((artist, index) => (
              ...
              >
                <div className="flex">
                  <div className="w-[40px] h-[40px] bg-divider-1 rounded-[50px] overflow-hidden">
                    {artist.imageUrl && (
                      <img
                        src={artist.imageUrl}
                        alt={artist.nameKr}
